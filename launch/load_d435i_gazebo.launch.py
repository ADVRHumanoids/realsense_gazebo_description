...
def launch_setup(context, *args, **kwargs):
    ...
    if (LaunchConfiguration("align_depth").perform(context).lower() == "true"):
        ...
        # Publish pointcloud only if enabled, as it is costly to compute
        if (LaunchConfiguration("publish_pointcloud").perform(context).lower() == "true"):
    ...
    # IMU bridging
    bridge_topics.append(
        f'/{LaunchConfiguration("name").perform(context)}/imu@sensor_msgs/msg/Imu[gz.msgs.IMU'
    )
...
def generate_launch_description():
    ...
    #  (see sensors mounted on _d435i.gazebo.xacro file). The standard gazebo plugin
